...
int iv = (ms_AIR - ms_H2O)/10;                    // system defined variable for moisutre level granularity (for plant profiling)
      ...
      if (current_soil_status > soil_status_water) {
        ...
        while ( (current_soil_status < soil_status_target) && (lastwatercycle < 5) ) {
          ...
          if (lowDeltaCount <= 3) {
            ...
            if (opmode == 0) { digitalWrite(digIO1, HIGH); }        // only turn on digital output if soil mositure sensor is not in self-watering pot mode
            
            // water for x seconds (can place any integer in here)
            water_now(pumpPin1, pumpPin2, water_time);
